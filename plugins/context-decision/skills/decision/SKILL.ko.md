...
같은-major context-core와 함께 쓴다. semantic owner는 대화를 재-audit하거나 쓰지 않고 core만 쓴다. Read-only command에는 host inventory가 없다. Init/capture는 인접 manifest, actual core digest, schema/doctor를 검증한다. Caller inventory는 low-level compatibility용이다.
...
decision·scope·lifecycle effect를 직접적·명시적·무조건적으로 확정한 선택은 semantic approval이다. 저장 파일의 렌더링 본문이나 별도 capture 질문은 없다. 승인 뒤 내부 preview의 frozen receipt에 semantic delta가 없음을 확인하고 `approval_digest`를 그대로 같은 응답의 apply에 전달한다. transport detail은 노출하거나 요구하지 않는다. 단순 확인·칭찬·조건·수정 요청·화제 전환은 승인이 아니다. delta면 write를 보류하고 그 차이만 확인하며 승인 뒤 재생성하지 않는다.
