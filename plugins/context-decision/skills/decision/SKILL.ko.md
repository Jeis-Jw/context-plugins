...
2. `check`는 metadata로 후보를 줄인 뒤 관련 Current DEC의 canonical `Decision`·`Rationale`·`Rejected alternatives`만 반환한다. 기존 `결정`·`취지`·`반려대안`은 legacy read/round-trip alias다. 실제 본문을 읽고 `new|same|supporting|rationale_changed|conflict` 중 하나로 판정한다. 문장 유사도, hash, ID와 metadata는 의미 판정 근거가 아니다.
...
11. 사용자가 scope의 현재 결정을 읽기용 명세로 요청하면 `spec-view --scope ...`를 사용한다. exact·ancestor·descendant Current DEC의 canonical `Decision`·`Rationale`만 조립하며 legacy 한국어 heading은 canonical key로 읽는다. History, 승인과 저장은 포함하지 않는다.
