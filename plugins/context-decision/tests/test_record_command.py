"""One-call ``record`` and deterministic sibling-core resolution.

record = preview + unchanged apply in one process, gated by ``--approved``.
Without ``--core-cli`` the single same-major sibling context-core is resolved
...
class RecordCommandTests(unittest.TestCase):
    ...
    def test_record_refuses_missing_or_ambiguous_sibling_core_before_writing(self) -> None:
        packages = self.repo / "packages"
        copied_owner = packages / "context-decision"
        shutil.copytree(ROOT / "plugins/context-decision", copied_owner, ignore=shutil.ignore_patterns("__pycache__"))
