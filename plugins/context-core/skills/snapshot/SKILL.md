...
Resolve `/loaded/...` from this file's own path in the skill catalog. Open items and next steps take one item per line (a `- ` bullet is optional); the context is one paragraph.
