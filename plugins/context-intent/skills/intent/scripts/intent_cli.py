...
def required_core_surface(value: str, *, expected_sha256: str | None = None) -> pathlib.Path:
    ...
    try:
        ...
        raise IntentError("core_surface_unavailable", "the compatible context-core public CLI is unavailable", {"required_plugin": dict(REQUIRED_PLUGIN)}, EXIT_CONFLICT) from error
    suffix = pathlib.PurePosixPath(REQUIRED_PLUGIN["entrypoint"]).parts
    if (
...
def validate_core_schema_handshake(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise IntentError("core_incompatible", "context-core schema, protocol, feature, or required command handshake is incompatible", {"required_plugin": dict(REQUIRED_PLUGIN), "required_commands": sorted(required_commands)}, EXIT_CONFLICT)
    return value
