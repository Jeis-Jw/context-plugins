...
def _substantive(value: Any, *, maximum: int, field: str) -> str:
    ...
    if not result or result in PLACEHOLDERS or len(result) > maximum:
        raise TermError("schema_invalid", f"{field} is empty, placeholder, or too long")
    return result
...
def validate_transport_candidate(candidate: Any) -> dict[str, Any]:
    ...
    if len(canonical_json(candidate).encode("utf-8")) > MAX_CANDIDATE_BYTES:
        raise TermError("candidate_too_large", "candidate exceeds the 16 KiB protocol budget", exit_code=EXIT_CONFLICT)
    ...
    for kind, value in owner_inputs.items():
        ...
        if len(canonical_json(value).encode("utf-8")) > MAX_OWNER_INPUT_BYTES:
            raise TermError("owner_input_too_large", "owner input exceeds the 2 KiB protocol budget", {"kind": kind}, EXIT_CONFLICT)
...
def validate_candidate_batch(batch: Any) -> dict[str, Any]:
    ...
    if batch_bytes > MAX_CANDIDATE_BYTES:
        raise TermError("candidate_batch_too_large", "candidate batch exceeds the 16 KiB protocol budget", exit_code=EXIT_CONFLICT)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    decline.add_argument("--candidate", required=True)
    ...
    same.add_argument("--successor-candidate", required=True)
