...
def write_preflight(root: Path, state: str = "ready", *, entrypoint: Path | None = None) -> tuple[Path, Path]:
    ...
    doctor.write_text(json.dumps({"schema": "context-core-doctor/v1", "owner": "context-core", "supported_protocols": ["context-common/v2"], "repository_state": state, "root": "context/", "issues": [], "warnings": [], "plugin_version": "0.5.1", "entrypoint": str((entrypoint or CORE_CLI_PATH).resolve()), "protocol": "context-common/v2"}), encoding="utf-8")
