...
class TermPluginContractTests(unittest.TestCase):
    def test_public_init_feature_handshake_bootstraps_through_core(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue((repo / "context/term/term.index.md").is_file())
    ...
    def test_public_claim_decline_search_read_are_byte_noop(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            search = helpers.run_cli(repo, "search", "--signal", helpers.term_cli.SIGNAL, "--query", "BFF", *preflight, "--json")
